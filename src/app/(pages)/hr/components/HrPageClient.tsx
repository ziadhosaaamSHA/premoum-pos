...
export default function HrPage() {
  ...
  const employeeColumns: DataTableColumn<EmployeeRow>[] = [
    { header: "الاسم", cell: (employee) => employee.name },
    { header: "الدور", cell: (employee) => employee.role },
    { header: "الهاتف", cell: (employee) => employee.phone || "—" },
    {
      header: "الحالة",
      cell: (employee) => (
  ...
  const attendanceColumns: DataTableColumn<AttendanceRow>[] = [
    { header: "الموظف", cell: (attendance) => attendance.employee },
    { header: "الدخول", cell: (attendance) => formatDateTime(attendance.checkIn) },
    { header: "الخروج", cell: (attendance) => formatDateTime(attendance.checkOut) },
    { header: "الحالة", cell: (attendance) => <span className="badge neutral">{attendance.status}</span> },
  ];

  const shiftColumns: DataTableColumn<ShiftRow>[] = [
    { header: "الشيفت", cell: (shift) => shift.name },
    { header: "الوقت", cell: (shift) => shift.time },
    { header: "الموظفون", cell: (shift) => shift.staffCount },
    {
      header: "الحالة",
      cell: (shift) => (
  ...
  const payrollColumns: DataTableColumn<PayrollRow>[] = [
    { header: "الموظف", cell: (item) => item.employee },
    { header: "البند", cell: (item) => statusLabel(item.type) },
    { header: "القيمة", cell: (item) => money(item.amount) },
    { header: "التاريخ", cell: (item) => item.date },
  ];

  const leaveColumns: DataTableColumn<LeaveRow>[] = [
    { header: "الموظف", cell: (leave) => leave.employee },
    { header: "من", cell: (leave) => leave.from },
    { header: "إلى", cell: (leave) => leave.to },
    {
      header: "الحالة",
      cell: (leave) => (
  ...
  const userShiftColumns: DataTableColumn<UserEmployeeShiftRow>[] = [
    { header: "الموظف", cell: (row) => row.employeeName },
    {
      header: "المستخدم",
      cell: (row) => (
        <>
    ...
    { header: "الأدوار", cell: (row) => row.roles.join("، ") || "—" },
    {
      header: "حالة الحساب",
      cell: (row) => (
    ...
    { header: "عدد الورديات", cell: (row) => row.shiftsCount },
    { header: "إجمالي الدقائق", cell: (row) => row.totalShiftMinutes },
    { header: "آخر وردية", cell: (row) => (row.lastShiftStart ? formatDateTime(row.lastShiftStart) : "—") },
  ];
        ...
        <div className="subtab-panel active">
          <div className="card wide">
            ...
            <DataTable
              ...
              getRowProps={(employee) => ({ "data-status": employee.status })}
              actions={(employee) => ({
      ...
      {activeTab === "attendance" && (
        <div className="subtab-panel active">
          <div className="card wide">
            ...
            <DataTable
              id="hr-attendance-table"
              rows={filteredAttendance}
              columns={attendanceColumns}
              getRowKey={(attendance) => attendance.id}
              actions={(attendance) => ({
                onView: () => openAttendanceModal("view", attendance.id),
                onEdit: () => openAttendanceModal("edit", attendance.id),
                onDelete: () => deleteAttendance(attendance.id),
        ...
        <div className="subtab-panel active">
          <div className="card wide">
            ...
            <DataTable
              ...
              getRowProps={(shift) => ({ "data-status": shift.status })}
              actions={(shift) => ({
      ...
      {activeTab === "userShifts" && (
        <div className="subtab-panel active">
          <div className="card wide">
            ...
            <DataTable
              id="hr-user-shifts-table"
              rows={filteredUserEmployees}
              columns={userShiftColumns}
              getRowKey={(row) => row.employeeId}
              actions={(row) => ({
                onView: () => setSelectedUserShiftEmployeeId(row.employeeId),
              })}
            />
          </div>
        </div>
